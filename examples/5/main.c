  ...
  for (int i = 0; i < device_count; i++) {
    ...
    */
    ...
    printf("OpenMP took %lu ms (%.1f GFLOPS).\n", mp_result.time_us,
           (double)operation_count / mp_result.time_us / 1000);
    printf("OpenCL took %lu ms (%.1f GFLOPS).\n", cl_result.time_us,
           (double)operation_count / cl_result.time_us / 1000);
...
struct cl_result run_device(cl_device_id device_id, const cl_float *a,
  ...
  FILE *aplusb_file = fopen("../examples/4/matrix-multiply.cl",
                            "rb"); // Откроем файл
  ...
  size_t local_work_size[] = {TILE, TILE};
  cl_event event;
